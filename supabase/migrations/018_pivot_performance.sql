-- ============================================================
--  Performance do dre_realizado_pivot.
--  Problema: os predicados de permissão (is_admin(), user_restringe_*,
--  permitidas) eram avaliados POR LINHA da MV (~7.600 linhas/ano), cada um
--  batendo em profiles/usuario_* → ~1,5s por chamada. Na visão "Ano sobre
--  ano" (3 anos concorrentes) + orçamento, estourava o statement_timeout (8s).
--  Correção: reescrever em PL/pgSQL calculando as permissões UMA vez em
--  variáveis; o filtro por linha vira simples pertencimento a array.
--  Semântica idêntica à versão anterior.
-- ============================================================

create or replace function public.dre_realizado_pivot(p_empresa smallint, p_ano integer)
...
declare
  ...
  v_classifs   text[];
begin
  -- Sem acesso à empresa → nada.
  if not v_tem then
    return;
  end if;

  -- Bloco de empresa: partes que não dependem da linha, resolvidas uma vez.
  v_emp_const := v_admin
  ...
  -- Filiais liberadas ao usuário (única parte por-linha do bloco de empresa).
  select array_agg(uf.cd_empresa) into v_filiais
  from usuario_filiais uf where uf.user_id = auth.uid();

  -- Classificações permitidas (só importam quando não é admin).
  if not v_admin then
